    ...
    <div>
      ...
      <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter ? 'bg-primary text-white' : ''}`} onClick={() => setShowFilter(prev => !prev)}>Filters</button>
        <div className={`flex flex-col gap-4 text-sm text-gray-600 ${showFilter ? 'flex':'hidden sm:flex'}`}>
          ...
          {filterDoc.length === 0 && (<p className="text-red-500 text-center mt-4">Doctors are not available for this specialty.</p>)}
        </div>
        <div className='w-full grid grid-cols-auto gap-4'>
          {
            filterDoc.map((item, index) => (
              <div onClick={() => navigate(`/appointment/${item._id}`)} className='border border-blue-200 rounded-xl  cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
                  <img className='bg-blue-50 rounded-t-xl' src={item.image} alt="" />
                  <div className='p-4'>
                      <div className='flex items-center gap-2 text-sm  text-center tex-green-500'>
                          <p className='w-2 h-2 bg-green-500 rounded-full'> </p><p className='text-xs'>Available</p>
                      </div>
